...
## Assignment Operator {.unnumbered}
...
## Branching and If/Then {.unnumbered}

### Decisions Based on One Scalar Value {.unnumbered}
...
### Series of Separate Decisions Over a Vector of Values {.unnumbered}
...
There are so many functions in R that it may be better to use the [stackoverflow.com](https://stackoverflow.com/questions/tagged/r) Q&A to find the ones you need (as of 2022-05-26 there are 450,000 R questions there).  Here are just a few of the multitude of handy R functions.  The first functions listed below return the R missing value `NA` if any element is missing.  You can specify `na.rm=TRUE` to remove `NA`s from consideration first, so they will not cause the result to be `NA`.  Must functions get their arguments (inputs) in () after the function name.  Some functions like `%in%` are binary operators whose two arguments are given on the left and right of `%in%`.
...
One can specify sizes, layouts, captions, and more using `Quarto` markup.  Captions are ignored unless a figure is given a label.  Figure labels must begin with `fig-`.  The figure can be cross-referenced elsewhere in the document using for example `See \@fig-scatterplot`.  Here is example syntax.

| ```{r}
| #| label: fig-myplot
| #| fig-cap: An example caption (use one long line for caption)
| #| fig-scap: short caption (used only by addcap())
| #| fig-height: 3
| #| fig-width: 4
| plot(1:7, abs(-3 : 3))
| ```
...
For chunks having `#| label: fig-` you can automatically have `knitr` call `addCap` at the start of a chunk, extracting the needed information, if you run the `reptools` function `hookaddcap()` in a chunk before the first chunk that produced a graph.  This procedure is used through this document.
...
Set up for automatic create of list of figures.

```{r setuphook}
# hookaddcap is in reptools
...
::: {.column-margin}
To make `mermaid` tooltips work the following needs to be placed in your script.  This will keep text in the tooltip as fixed-width font so columns will line up.
...
dataOverview(support, baselabel='dataov')
```
...
maketabs(plot(w, bvspace=2.5), baselabel='pdesc', cap=cap)
```
...
cap <- paste('plotly plot(describe) output for', c('categorical', 'continuous'),
             'variables')
maketabs(plot(w, bvspace=2.5), wide=TRUE, baselabel='pldesc', cap=cap)
```
...
maketabs(
  ...
  wide=TRUE, baselabel='summarym', cap=c('', '', '-', '-'))
```
...
names(gg) <- nam
maketabs(gg, baselabel='curverep')
```
...
Many functions in the `Hmisc` and `rms` packages produce `plotly` graphics directly.  One of the most unique functions is [dotchartpl](https://www.rdocumentation.org/packages/Hmisc/versions/4.6-0/topics/dotchartpl).  These two package's functions using `plotly` try to compute optimal figure heights and widths, but it is usually better to let `plotly` auto-size the plots.  Putting `options(plotlyauto=TRUE)` will override the computed dimensions and force `plotly` to auto-size.  Putting this command in your `.Rprofile` file in the home directory makes this easy.
...
* [Stay close to the raw data](https://stackoverflow.blog/2022/03/03/stop-aggregating-away-the-signal-in-your-data). Analyze the rawest form of the data when possible.  Don't convert inherently longitudinal data into time-to--event data.
...
#| fig-scap: "Nonparametric smooth estimated relationships between several baseline variables and hospital mortality"
#| fig-height: 4.5
#| fig-width: 6.25
...
#| fig-scap: "Moving nonparametric and flexible parametric estimates of mortality by age"
...
#| fig-scap: "1-year mortality stratified by disease group"
...
#| fig-scap: "Moving Kaplan-Meier and HARE estimates"
...
#| fig-scap: "Predicted mean glycohemoglobin vs. age using a parametric model"
require(rms)
...
```{r nhghq}
#| label: fig-nhghq
#| fig-cap: "Smooth age effects on three quartiles of `r hba1c`"
...
## Another Replacement for Table 1
...
```{r allcat}
#| label: fig-allcat
#| fig-cap: "Kaplan-Meier estimates of 1y and 2y incidence stratified separately by a series of discrete predictors"
#| column: page-inset-left
#| fig-height: 5.5
#| fig-width: 9
...
maketabs(g, baselabel='catkmtabs')
```
...
#| fig-scap: "Simulation results for absolute correlation coefficient"
